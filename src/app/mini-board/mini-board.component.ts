...
export class MiniBoardComponent implements OnInit {
  ...
  refreshNode(node: Node, endPos, delta) {
    ...
    if (node.leftLine) node.leftLine.transition().duration(this.duration).attr('x1', endPos.x).attr('y1', endPos.y).attr('x2', endPos.x - delta.x).attr('y2', endPos.y + delta.y).style('z-index', -1)
    if (node.rightLine) node.rightLine.transition().duration(this.duration).attr('x1', endPos.x).attr('y1', endPos.y).attr('x2', endPos.x + delta.x).attr('y2', endPos.y + delta.y).style('z-index', -1)
    node.x = endPos.x
    node.y = endPos.y
