...
export class BoardComponent implements OnInit {
  ...
  drag(node) {
    let x = Math.min(
      Math.max(d3.event.x, node.left ? node.left.x + 10 : 10),
      node.right ? node.right.x : 990
    );
    const y = Math.min(
      Math.max(d3.event.y, node.parent ? node.parent.y + 10 : 10),
      node.left ? node.left.y - 10 : 990,
      node.right ? node.right.y - 10 : 990
    );
      ...
      if (node.value == null) {
        ...
        if (this.mode === 'violate') {
          ...
          this.remove.on('click', () => {
            if (this.tree.root.left.value === null && this.tree.root.right.value === null) {
              this.message = '루트 노드는 없앨 수 없습니다'
